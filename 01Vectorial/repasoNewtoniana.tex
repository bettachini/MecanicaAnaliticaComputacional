...
\title[Mecánica Newtoniana]{Mecánica Analítica Computacional\\[0.5cm] {\Huge Mecánica Newtoniana}}
...
\begin{itemize}
	\item por un escalar: \(m \vec{r}= m r_i \hat{i} + m r_j \hat{j}+ m r_k \hat{k}\)
	\pause 
	...
	\end{itemize}
	\pause 
	\item vectorial: \(\vec{r} \cross \vec{s} =
	\begin{vmatrix}
		\hat{i} & \hat{j} & \hat{k} \\
		r_i & r_j & r_k\\
		s_i & s_j & s_k
...
\vec{v} = 
\dv{r_1}{t} \hat{e}_1 + r_1 \dv{\hat{e}_1}{t} + 
\dv{r_2}{t} \hat{e}_2 + r_2 \dv{\hat{e}_2}{t} + 
\dv{r_3}{t} \hat{e}_3 + r_3 \dv{\hat{e}_3}{t}
\]
Tendríamos seis términos según la regla de la cadena.\\
}
\onslide<3->{
En el caso cartesiano \(\hat{x}, \hat{y}\) y \(\hat{z}\) no varían con \(t\)
\[
\vec{v}= \dot{\vec{r}}= 
\dv{x}{t} \hat{x}+
\dv{y}{t} \hat{y}+
...
\begin{columns}[c]
  ...
  \begin{column}{0.65\textwidth}
		...
		\begin{align*}
			...
			m \ddot{r} &= F_x = w_x- f = m g \sen{\theta} - \mu m g \cos{\theta}\\
