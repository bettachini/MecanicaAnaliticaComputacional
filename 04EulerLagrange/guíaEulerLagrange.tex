...
\item
	...
	Resultado:\\
	\(
		l m \left(g \sin{\left(\varphi \right)} + l \ddot{\varphi} + \cos{\left(\beta + \varphi \right)} \ddot{x}\right) = 0 \\
		M g \sin{\left(\beta \right)} - M \ddot{x} + g m \sin{\left(\beta \right)} + l m \sin{\left(\beta + \varphi \right)} \dot{\varphi}^{2} - l m \cos{\left(\beta + \varphi \right)} \ddot{\varphi} - m \ddot{x} = 0
	\)
