...
\textbf{Aro y polea}\\
	...
	\begin{enumerate}
		...
		\item Describa la ligadura conemplando el ángulo del aro medido desde la horizontal.
		\item Obtenga la ecuación de Euler-Lagrange para la dinámica.\\
		Resultado:
		$R \left(- 2 M R \ddot{\theta} + M g \cos{\left(\theta \right)} - R m \ddot{\theta} - R m_{a} \ddot{\theta} - g m\right) = 0$
	\end{enumerate}
\end{minipage}
...
\begin{enumerate}
	...
	\begin{enumerate*}[
		...
		\item \(y\), la de la partícula con masa \(m_2\),
		\item y para \(\theta\)
