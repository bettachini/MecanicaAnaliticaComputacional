\documentclass[aspectratio=169]{beamer} % proyector 16:9 | Ref:  https://en.wikibooks.org/wiki/LaTeX/Presentations
\beamertemplatenavigationsymbolsempty %% Sin barra navegación
\usetheme{Madrid}

\usepackage{amsmath} % Símbolos matemáticos

%% Spanska!
\usepackage[spanish]{babel}
\def\spanishoptions{argentina}


%% inclusión de gráficas
\usepackage{graphicx}	% instalar ghostscript-x para que el dvi muestre los eps
\graphicspath{ {./figs/} {./imagenes} {../figuras/}}
\usepackage{tikz}


\setbeamertemplate{footline}{%
}


\setbeamertemplate{frametitle}{%
   \insertframetitle%
}
\setbeamercolor{frametitle}{fg=black} % Set text color to white



%% Main matter
\begin{document}

% Cover slide
\usebackgroundtemplate{
  \includegraphics[width=\paperwidth, height=\paperheight]{ersteCAIFE}
} %% https://mprnotes.wordpress.com/2009/08/14/changing-background-image-of-latex-beamer/
\begin{frame}[plain] % plain required for correct page numbering
\end{frame}

\definecolor{caifeblue}{HTML}{18204f}
\setbeamercolor{title}{fg=white,bg=caifeblue}
\setbeamercolor{section in toc}{fg=caifeblue}

% Normal blocks
\setbeamercolor{block title}{fg=white,bg=caifeblue}
\setbeamercolor{block body}{fg=black,bg=caifeblue!5!white}



%% Title slide
\usebackgroundtemplate{
  \includegraphics[width=\paperwidth, height=\paperheight]{titleBackground}
	}	% unset background % https://tex.stackexchange.com/questions/201013/how-to-include-a-background-image-to-only-one-page-of-a-beamer-presentation
\begin{frame}[plain] 
  %\titlepage
 
	\vspace{0.1cm}
	
	\makebox[\textwidth][c]{%
  \begin{beamercolorbox}[wd=0.85\textwidth,sep=4pt,center,rounded=true,shadow=true]{title}
    \usebeamerfont{title}\textbf{Introducción temprana de la mecánica computacional\\
    en cursos de electrostática y mecánica analítica}
  \end{beamercolorbox}%
}
  \vspace{0.1cm}

	\begin{beamercolorbox}[wd=0.21\textwidth,sep=0pt,center,rounded=true,shadow=true]{title}
    \usebeamerfont{title} {\normalsize\textbf{INTEGRANTES }}
  \end{beamercolorbox}
 	
	\vspace{-0.3cm}
 
	\begin{flushleft}
	$\bullet$ Víctor A. Bettachini, Dep. de Ingeniería e Investigaciones Tecnológicas, UNLaM
	\\
	$\bullet$ Mariano A. Real, Inst. de Calidad Industrial, UNSaM -- INTI
	\\
	$\bullet$ Edgardo Palazzo, Dep. de Materias Básicas, UTN--FRA
  \end{flushleft}

  \vspace{0.1cm}

  % Another box
  \begin{beamercolorbox}[wd=0.21\textwidth,sep=0pt,center,rounded=true,shadow=true]{title}
    \usebeamerfont{title} {\normalsize\textbf{SÍNTESIS }}
  \end{beamercolorbox}

	\vspace{-0.3cm}

	\begin{flushleft}
	La ejercitación basada en la edición de código extendió el alcance de dos asignaturas:
	\\
	$\bullet$ Modelar dispositivos industriales realistas con la mecánica analítica.\\ 
	$\bullet$ Mayor comprensión conceptual de la electrostática al evitar matemática avanzada. 
  \end{flushleft}


\end{frame}

% Reset counter after title page
\setcounter{framenumber}{0} % This makes next frame "01"


%% Standard slide
\usebackgroundtemplate{
  \includegraphics[width=\paperwidth, height=\paperheight]{slideBackground}
} %% https://mprnotes.wordpress.com/2009/08/14/changing-background-image-of-latex-beamer/

\begin{frame}
	\frametitle{Temario}
	\begin{columns}
		\begin{column}{0.4 \textwidth}
			\begin{block}{}
				\vspace{0.2cm}
				\tableofcontents
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\section{Herramientas que definen el aula}
\begin{frame}
	\frametitle{Temario}
	\begin{columns}
		\begin{column}{0.4 \textwidth}
			\begin{block}{}
				\vspace{0.2cm}
				\tableofcontents[currentsection]
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\begin{frame}
	\frametitle{Herramientas omnipresentes}
  %\begin{beamercolorbox}[wd=0.21\textwidth,sep=0pt,center,rounded=true,shadow=true]{frametitle}
  %  \usebeamerfont{title}frametitle
  %\end{beamercolorbox}%
	\begin{columns}
	\begin{column}{0.47 \textwidth}
			\begin{block}{Inicio s. XIX: pizarrón y papel}
				\includegraphics[width= \columnwidth]{../figuras/1930s-1940s-man-teacher-professor-vintage-images.jpg}
			\end{block}
		\end{column}
		\pause
		\begin{column}{0.45 \textwidth}
			\begin{block}{Fin s. XX: computadora como proyector}
				\includegraphics[width= \columnwidth]{../figuras/systemsEngineering.png}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\begin{frame}
	\frametitle{Subutilización de la tecnología de la información}
	\begin{columns}
	\begin{column}{0.47 \textwidth}
			\pause
			\begin{block}{La resistencia al cambio}
				\includegraphics[width= \columnwidth]{../figuras/reglaCalculadora}
			\end{block}
		\end{column}
		\pause
		\begin{column}{0.45 \textwidth}
			\begin{block}{Décadas de atraso}
				\includegraphics[width= \columnwidth]{../figuras/trap2}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



%\section{Aula centrada en el código}
%\begin{frame}
%	\frametitle{Temario}
%	\begin{columns}
%		\begin{column}{0.4 \textwidth}
%			\begin{block}{}
%				\vspace{0.2cm}
%				\tableofcontents[currentsection]
%			\end{block}
%		\end{column}
%	\end{columns}
%\end{frame}


\section{UNLaM | Mecánica analítica}
\begin{frame}
	\frametitle{Temario}
	\begin{columns}
		\begin{column}{0.4 \textwidth}
			\begin{block}{}
				\vspace{0.2cm}
				\tableofcontents[currentsection]
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\begin{frame}
	\frametitle{Mecánica analítica en UNLaM inició en pandemia}
	\begin{columns}
	\begin{column}{0.4 \textwidth}
	% \begin{column}{0.37 \textwidth}
			\begin{block}{Puente matemática -- aplicadas}
				\includegraphics[width= \columnwidth]{../figuras/correlativas_2}
			\end{block}
		\end{column}
		\pause
		\begin{column}{0.55 \textwidth}
			\begin{block}{Tras una computadora... ¡Automaticémos!}
				\includegraphics[width= \columnwidth]{../figuras/hard}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}


\begin{frame}
	\frametitle{Cuaderno Jupyter: teoría + código ejecutable}
	%\pause
	\begin{block}{}
	% \begin{block}{On-line programmable notebook: text + equations + code}
		\includegraphics[width= \columnwidth]{screenshot_JupyterLab}
	\end{block}
\end{frame}


\begin{frame}
\frametitle{Google Colaboratory: cuadernos Jupyter en la nube}
	% \pause
	\begin{block}{}
		% \begin{itemize}[<+->]
		\begin{itemize}
			\item Docente puede editar y comentar cuaderno del alumno.
			% \item It runs Jupyter notebooks online, and it's free.
			% \item Trabajo colaborativo de múltiples estudiantes.
		\end{itemize}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.85\textwidth]{comentariosColab}
		\end{center}
	\end{block}
	% \pause
\end{frame}


%\begin{frame}
%	\frametitle{\LaTeX \,: notación matemática concisa}
%	% \pause
%	\begin{block}{}
%		\begin{itemize}
%			\item \LaTeX \, sigue el estandar de la American Mathematical Society
%		\end{itemize}
%		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
%		\begin{center}
%			\includegraphics[width= 0.8\textwidth]{clase1péndulo}
%		\end{center}
%	\end{block}
%	% \pause
%\end{frame}


\begin{frame}
	\frametitle{Repositorio GitHub}
	% \pause 
	\begin{block}{}
		% \begin{itemize}[<+->]
		\begin{itemize}
			\item Cuaderno Jupyter: GitHub $\xrightarrow{carga}$ Google Colaboratory
			% \item \textbf{Actualizado} por docente. \textbf{Clonado} por el alumno.
		\end{itemize}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.8\textwidth]{repositorioGithub2}
		\end{center}
	\end{block}
	% \pause
\end{frame}



%\begin{frame}
%	\frametitle{1er clase | Matplotlib: precise and reproducible graphics}
%	% \pause
%	\begin{block}{}
%		\begin{itemize}
%			\item Explicit Python code for producing graphics. Students are enticed to play with it.
%		\end{itemize}
%		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
%	\includegraphics[width= 0.8\textwidth]{clase1gráficos}
%	\end{block}
%	% \pause
%\end{frame}





%\begin{frame}
%	\frametitle{1er clase | SymPy: álgebra y análisis simbólico}
%	% \pause
%	\begin{block}{}
%		\begin{itemize}
%			\item El esfuerzo del estudiante se centra en la nueva temática
%		\end{itemize}
%		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
%		\begin{center}
%			\includegraphics[width= 0.9\textwidth]{clase3sympy}
%		\end{center}
%	\end{block}
%	% \pause
%\end{frame}


\begin{frame}
	\frametitle{3er clase | SymPy: álgebra y análisis simbólico}
	% \pause
	\begin{block}{}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.9\textwidth]{clase5EulerLagrange.png}
		\end{center}
	\end{block}
	% \pause
\end{frame}


%\begin{frame}
%	\frametitle{Tools | 4th class | Automatation of resolutions}
%	% \pause
%	\begin{block}{}
%		\begin{itemize}
%			% \item No heavy calculations consuming the energy of the students.
%			\item Mathematical complexity doesn't limit the scope of tackled mechanical problems.
%		\end{itemize}
%		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
%	\includegraphics[width= \textwidth]{clase4Aceleraciones}
%	\end{block}
%	% \pause
%\end{frame}


\begin{frame}
	\frametitle{5ta clase | SciPy: cálculo numérico}
	% \pause
	\begin{block}{}
		%\begin{itemize}
		%	\item Explicit solutions.
		%\end{itemize}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.9\textwidth]{clase5Soluciones.png}
		\end{center}
	\end{block}
	% \pause
\end{frame}


\begin{frame}
	\frametitle{5ta clase | matplotlib: análisis gráfico de resultados}
	% \pause
	%Class 5: analysis of results
	\begin{block}{}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.9\textwidth]{clase5Representación}
		\end{center}
	\end{block}
	% \pause
\end{frame}



\begin{frame}
	\frametitle{7ma clase | Esfuerzos y dinámica de máquinas}
	\begin{center}
		\begin{minipage}{0.6\linewidth}
			\begin{block}{}
				\begin{center}
					\includegraphics[width= 0.55\columnwidth]{clase7esquema.png}\\
					\includegraphics[width= \columnwidth]{robotArm.png}
				\end{center}
			\end{block}
		\end{minipage}
	\end{center}
\end{frame}


\begin{frame}
	\frametitle{8va clase | GitHub Copilot: código de un LLM}
	% \pause
	%Class 7: Adding complexity
	\begin{block}{}
		\begin{itemize}
			\item Tras dominar lo básico los estudiantes pueden aprovecharse de la IA.
			%\item Gradual increase in complexity of simulated mechanical devices.
		\end{itemize}
		% \uncover<4->{\includegraphics[height= 2cm]{ucarecdn}}
		\begin{center}
			\includegraphics[width= 0.85\textwidth]{copilot}
		\end{center}
	\end{block}
	% \pause
\end{frame}


%\begin{frame}
%	\frametitle{Rudimentos de mecánica computacional}
%	\begin{block}{Mecánica analítica aplicada}
%		\begin{itemize}
%			\item Dispositivos como conjunto de cuerpos rígidos $\rightarrow$ modelo matemático.
%			\item Euler-Lagrange $\rightarrow$ ecuaciones diferenciales de esfuerzos y dinámica.
%			\item Cálculo numérico $\rightarrow $ gráficas y magnitudes para orientar el diseño.
%		\end{itemize}
%	\end{block}
%	\pause
%	\begin{block}{El alumno ``construye" su competencia}
%		Papert (1980) ``El mejor aprendizaje tiene lugar cuando el sujeto toma el mando''
%		\begin{itemize}
%		% \begin{itemize}[<+->]
%			\item Problemas de ejemplo con código provisto.
%			\item \textbf{Reciclado} para resolver guía de ejercicios.
%			\item Gradualmente, va generando código propio.
%			\item Con IA acelera el proceso.
%			\item Repositorio con código propio.
%		\end{itemize}
%	\end{block}
%\end{frame}


\begin{frame}
	\frametitle{Valorar el tiempo de docentes y alumnos}
	\begin{block}{}
	  \begin{columns}[c]
			\begin{column}{0.28\textwidth}
				\includegraphics[width= \columnwidth]{1930s-1940s-man-teacher-professor-vintage-images}
			\end{column}
			\begin{column}{0.7\textwidth}
				Pizarrón/presentación
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item Profesor: prepara lecciones \(\xrightarrow{de\, memoria}\) pizarrón/presentación
					\item Estudiante: pizarrón/presentación \(\xrightarrow{transcribe}\) cuaderno
					\item Práctica: \textbf{rehacer} los mismo cálculos, diagramas, etc\dots
					\item Lento y reiterativo \(\implies \downarrow\) interés en la asignatura
				\end{itemize}
			\end{column}
		\end{columns}
	\end{block}
	\pause
	\begin{block}{}
	Licklider (1957): 85\% de ``pensar'' es lo ``mundano'': organizar información, calcular, dibujar...
	  \begin{columns}[c]
			\begin{column}{0.3\textwidth}
				\includegraphics[width= \columnwidth]{"Screenshot 2023-09-18 at 12-24-03 Google Colaboratory"}
			\end{column}
			\begin{column}{0.7\textwidth}
				Cuaderno Jupyter/repositorio GitHub
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item Profesor: ideas \(\xrightarrow{actualiza}\) código/teoría en repositorio
					\item Estudiante: repositorio del curso \(\xrightarrow{clona}\) uno propio
					\item Práctica: \textbf{reciclar} código para resolver nuevos problemas
				\end{itemize}
			\end{column}
		\end{columns}
	\end{block}
\end{frame}






%\begin{frame}
%	\frametitle{UNLaM | Aula invertida}
%	\begin{columns}[c]
%    \begin{column}{0.6 \textwidth}
%			\includegraphics[width= \columnwidth]{aulaInvertidaHorario}
%    \end{column}
%		\begin{column}{0.4 \textwidth}
%			\pause
%			La encuesta y gráficos de sus resultados
%      \begin{itemize}[<+->]
%      % \begin{enumerate}[<+->]
%         \item Campos magnéticos
%         \item Campos eléctricos
%         \item Temperatura
%				 \item Vibraciones
%				 \item Presión atmosférica
%      \end{itemize}
%      % \end{enumerate}
%    \end{column}
%  \end{columns}
%\
%\end{frame}




\section{UTN--FRA | Física II}
\begin{frame}
	\frametitle{Temario}
	\begin{columns}
		\begin{column}{0.4 \textwidth}
			\begin{block}{}
				\vspace{0.2cm}
				\tableofcontents[currentsection]
			\end{block}
		\end{column}
	\end{columns}
\end{frame}


\begin{frame}
	\frametitle{El problema inverso: insuficiente matemática}
	\begin{columns}[c]
		\begin{column}{0.4 \textwidth}
			\begin{block}{Física II en UTN--FRA}
				\includegraphics[width=\columnwidth]{../figuras/utn_fra_ingMec_correlativasF2_2.pdf}
			\end{block}
		\end{column}
		\pause
		\begin{column}{0.55 \textwidth}
			\begin{block}{Salvando limitaciones de los alumnos}
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item Carecen del análisis para estudiar distribuciones de carga eléctrica $\rightarrow$ aproximado con discretas.
					\item Sin programación o cálculo numérico $\rightarrow$ funciones encapsuladas.
					\item Plantilla Jupyter para completar $\rightarrow$ estudio de casos inalcanzables con su matemática.
				\end{itemize}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}


\begin{frame}
	\frametitle{Líneas de campo de anillo cargado}
	\begin{columns}[c]
    \begin{column}{0.3 \textwidth}
			\begin{block}{Posición de cargas}
				\includegraphics[width=\textwidth]{imagenes/anillocode3.png}
			\end{block}
		\end{column}
    \begin{column}{0.3 \textwidth}
			\begin{block}{Cálculo del campo}
				\includegraphics[width=\textwidth]{imagenes/anillocode2.png}
				\includegraphics[width=\textwidth]{imagenes/anillocode1.png}
			\end{block}
		\end{column}
    \begin{column}{0.3 \textwidth}
			\begin{block}{Líneas de campo}
				\includegraphics[width=\textwidth]{imagenes/anillo.png}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\begin{frame}
	\frametitle{Saliendo del plano: dipolo eléctrico}
			\begin{block}{Curvas equipotenciales}
				\includegraphics[width=\textwidth]{imagenes/equipotenciales_superficies}
			\end{block}
\end{frame}



\begin{frame}
	\frametitle{Valoración de los alumnos de la experiencia}
	\begin{columns}[c]
		\begin{column}{.6 \textwidth}
			\begin{block}{}
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item Evita frustraciones: estudian problemas que podrían resolverlos analíticamente, pero fallarían mucho
					\item Genera interés: problemas realistas.
					\item Retroalimentación permite la prueba y error.
					\item Comprueban resultados de problemas símil examen.
				\end{itemize}
			\end{block}
		\end{column}
%		\pause
		%\begin{column}{.35 \textwidth}
			%\begin{block}{Docentes}
				%\begin{itemize}[<+->]
					%\item Registro de entrega en línea.
					%\item ``Ida y vuelta''  de cometarios y correcciones al tiempo que se resuelven los ejercicios.
				%\end{itemize}
			%\end{block}
		%\end{column}
	\end{columns}
\end{frame}


\section{Para concluir}
\begin{frame}
	\frametitle{Temario}
	\begin{columns}
		\begin{column}{0.4 \textwidth}
			\begin{block}{}
				\vspace{0.2cm}
				\tableofcontents[currentsection]
			\end{block}
		\end{column}
	\end{columns}
\end{frame}



\begin{frame}
	\frametitle{Código en cursos de ingeniería mecánica}
	\begin{columns}[c]
		\begin{column}{0.55 \textwidth}
			\begin{block}{Objeciones}
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item ``Los ingenieros mecánicos no programan.''
					\begin{itemize}
					% \begin{itemize}[<+->]
						\item Modificar código no es programar.
						\item Código: omnipresente en la industria.
					\end{itemize}
					\item ``No hay suficientes computadoras.''
					\begin{itemize}
					% \begin{itemize}[<+->]
						\item Las hay en hogares y trabajos (pandemia).
						\item Se probó en clase editar y ejecutar código en Google Colaboratory usando teléfonos.
					\end{itemize}
				\end{itemize}
			\end{block}
		\end{column}
		\pause
		\begin{column}{0.35 \textwidth}
			\begin{block}{Código en el aula}
				\begin{itemize}
				% \begin{itemize}[<+->]
					\item Análisis casos realistas.
					\item Del docente conferenciante $\rightarrow$ mentor ante ejercicios.
					\item Eficientizar tiempo: copiar en papel $\rightarrow$ reciclar código.
					\item Colaboración remota.
					\item Asistencia de IA.
				\end{itemize}
			\end{block}
		\end{column}
	\end{columns}
\end{frame}


% Closing slide
\usebackgroundtemplate{
  \includegraphics[width=\paperwidth, height=\paperheight]{lastCAIFE}
} %% https://mprnotes.wordpress.com/2009/08/14/changing-background-image-of-latex-beamer/
\begin{frame}[plain] % plain required for correct page numbering
\end{frame}


%%% Material extra
%%% Standard slide
%\usebackgroundtemplate{
%  \includegraphics[width=\paperwidth, height=\paperheight]{slideBackground}
%} %% https://mprnotes.wordpress.com/2009/08/14/changing-background-image-of-latex-beamer/
%
%
%\begin{frame}
%	\frametitle{Clase invertida | Ciclo semanal}
%	\begin{columns}[c]
%    \begin{column}{0.4\textwidth}
%			\begin{block}{}
%			%\begin{block}{Flipped classroom}
%				\includegraphics[width= \columnwidth]{flippedClassroomSchedule_castellano.png}
%			\end{block}
%    \end{column}
%    \begin{column}{0.55\textwidth}
%			\begin{block}{}
%				\begin{tabular}{lll}
%					\hline
%					Sincrónico & Teoría & Ejercicios\\
%					\hline
%					Antes & Leer y aplicar & Iniciarles\\
%					Durante & Consultas & Completarles\\
%					% Durante & Aclarar dudas & \begin{tabular}{@{}c@{}}Terminarles\\(semanal)\end{tabular}\\
%					Luego & \begin{tabular}{@{}l@{}}Aclaraciones\\adicionales\end{tabular} & \begin{tabular}{@{}l@{}}Ayuda de auxiliares\end{tabular}\\
%					\hline
%				\end{tabular}
%			\end{block}
%    \end{column}
%  \end{columns}
%	\pause
%	\begin{block}{}
%	% \begin{block}{New theory alongisde its worked examples in programmable notebooks}
%		\begin{itemize}
%			\item Publicación semanal de teoría y ejemplos en cuadernos Jupyter, y guía de ejercicios.
%			\item Ejercicios de la semana anterior \textbf{deben} entregarse para evaluación.
%			\item Las consultas \textbf{asincrónicas} en línea (24/7) son \textbf{públicas} para que otros las aprovechen.
%			\item Encuentros \textbf{sincrónicos} con auxiliares para finalizar ejercicios.
%%			%\item \textbf{Remote collaboration} on multi-user notebooks
%			%\item Weekly meetings to \textbf{synchronically} unfinished assignments with TA's assistance
%			% \item On a weekly basis these \textbf{must} be turned-in for scoring
%		\end{itemize}
%	\end{block}
%
%\end{frame}
%
%
%%\begin{frame}
%%	\frametitle{Methodology | Google Colaboratory: asynchronic remote assistance}
%%	\begin{block}{}
%%	% \begin{block}{Student's work can be commented and edited in Google Colaboratory}
%%		\includegraphics[width= \columnwidth]{comentariosColab}
%%	\end{block}
%%\end{frame}
%
%
%
%\begin{frame}
%	\frametitle{Microsoft Teams: seguimiento individualizado}
%	\begin{block}{Registro de ejercicios entregados}
%		\includegraphics[width= 0.9\columnwidth]{seguimiento}
%	\end{block}
%\end{frame}
%
%
%%\begin{frame}
%%	\frametitle{Methodology | Exams are just another assignment}
%%	\begin{block}{}
%%	% \begin{block}{The exam is just another excercise.}
%%		\begin{itemize}
%%			\item The students send their notebook file.
%%			\item Feedback is inserted in between the student's work.
%%		\end{itemize}
%%		\includegraphics[width= \columnwidth]{parcial}
%%	\end{block}
%%\end{frame}
%
%
%
%
%
%\begin{frame}
%	\frametitle{Difusión y acceso libre al material}
%	\begin{columns}[c]
%    \begin{column}{0.44 \textwidth}
%			\begin{block}{Repositorio GitHub}
%				\includegraphics[width= \columnwidth]{gitHubCAM}
%			\end{block}
%    \end{column}
%		\pause
%		\begin{column}{0.5 \textwidth}
%			\begin{block}{Journal Open Source Education}
%				\includegraphics[width= \columnwidth]{joseCAM}
%			\end{block}
%		\end{column}
%		\end{columns}
%\end{frame}
%


\end{document}
